...
import { leadRuns, agentRuns, leadPacks } from "@/drizzle/schema";
...
export async function createRun(params: CreateRunParams): Promise<{ runId: string }> {
  const [active] = await db
    ...
    .from(leadRuns)
    .where(or(eq(leadRuns.status, "running"), eq(leadRuns.status, "queued")));
...
export async function requestCancel(runId: string) {
  await db.update(leadRuns).set({ cancelRequested: true }).where(eq(leadRuns.id, runId));
}

export async function getRun(runId: string) {
  const [run] = await db.select().from(leadRuns).where(eq(leadRuns.id, runId));
  return run ?? null;
}

export async function touchHeartbeat(runId: string) {
  await db.update(leadRuns).set({ heartbeatAt: new Date() }).where(eq(leadRuns.id, runId));
}
...
async function computeRunStats(runId: string) {
  const [costRow] = await db
    ...
    .from(agentRuns)
    .where(eq(agentRuns.leadRunId, runId));

  const [packRow] = await db.select({ count: sql<number>`count(*)::int` }).from(leadPacks).where(eq(leadPacks.runId, runId));
