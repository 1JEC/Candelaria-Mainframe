import { db } from "@/db";
import { prospectLeads, prospectMailboxes, prospectReplies, prospectEnrollments } from "@/db/schema";
 ...
 * failure specifically (write to prospectOutbox, stop — never an error).
 */
export async function checkSendGates(ctx: SendGateContext): Promise<SendGateResult> {
  ...
  const [lead] = await db.select().from(prospectLeads).where(eq(prospectLeads.id, ctx.leadId));
  ...
  const replyConditions = [eq(prospectReplies.leadId, ctx.leadId)];
  if (domainPattern) replyConditions.push(ilike(prospectReplies.fromAddress, domainPattern));
  const [existingReply] = await db.select({ id: prospectReplies.id }).from(prospectReplies).where(or(...replyConditions));
  if (existingReply) {
    return { allowed: false, failure: { gate: 3, reason: "Er is al een reactie ontvangen van dit bedrijf — sequentie gestopt." } };
  }
  const [enrollment] = await db.select().from(prospectEnrollments).where(eq(prospectEnrollments.id, ctx.enrollmentId));
  ...
  const [mailbox] = await db.select().from(prospectMailboxes).where(eq(prospectMailboxes.id, ctx.mailboxId));
