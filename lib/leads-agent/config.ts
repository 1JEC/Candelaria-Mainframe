import { db } from "@/db";
import { prospectConfig } from "@/db/schema";
...
export async function getConfig<T = unknown>(key: ConfigKey): Promise<T> {
  const [row] = await db
    .select()
    .from(prospectConfig)
    .where(and(eq(prospectConfig.key, key), eq(prospectConfig.isActive, true)))
    .orderBy(desc(prospectConfig.version))
    .limit(1);
  if (!row) return DEFAULTS[key] as T;
  return row.valueJson as T;
}

export async function saveConfig(key: ConfigKey, value: Record<string, unknown>, actor: ProspectActor): Promise<number> {
  const [latest] = await db
    .select({ version: prospectConfig.version })
    .from(prospectConfig)
    .where(eq(prospectConfig.key, key))
    .orderBy(desc(prospectConfig.version))
    .limit(1);
  const nextVersion = (latest?.version ?? 0) + 1;

  await db
    .update(prospectConfig)
    .set({ isActive: false })
    .where(and(eq(prospectConfig.key, key), eq(prospectConfig.isActive, true)));

  await db.insert(prospectConfig).values({
    key,
    version: nextVersion,
    valueJson: value,
    isActive: true,
  ...
  await recordAudit({
    orgId: actor.orgId,
    userId: actor.userId,
    action: "prospecting_config_updated",
    entity: "prospect_config",
    entityId: key,
    meta: { version: nextVersion, scope: "global" },
  });
...
export async function listConfigVersions(key: ConfigKey) {
  return db
    .select({ version: prospectConfig.version, isActive: prospectConfig.isActive, updatedBy: prospectConfig.updatedBy, createdAt: prospectConfig.createdAt })
    .from(prospectConfig)
    .where(eq(prospectConfig.key, key))
    .orderBy(desc(prospectConfig.version));
}

export async function restoreConfigVersion(key: ConfigKey, version: number, actor: ProspectActor): Promise<number> {
  const [row] = await db
    .select()
    .from(prospectConfig)
    .where(and(eq(prospectConfig.key, key), eq(prospectConfig.version, version)));
  if (!row) throw new Error(`No version ${version} for config key ${key}`);
  return saveConfig(key, row.valueJson as Record<string, unknown>, actor);
}
