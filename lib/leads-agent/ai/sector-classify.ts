...
async function classifyBatch(batch: ClassifyCandidate[], runId?: string): Promise<ClassifyResult[]> {
...
export async function classifySectors(candidates: ClassifyCandidate[], runId?: string): Promise<ClassifyResult[]> {
  ...
  for (let i = 0; i < candidates.length; i += BATCH_SIZE) {
    ...
    results.push(...(await classifyBatch(batch, runId)));
  }
  return results;
}
