...
export async function processCandidateTask(runId: string, taskId: string, targetJson: string, startedBy: string | null) {
  ...
  if (candidate.website) {
    ...
    // A retried task (re-run on the same leadId after a mid-run failure —
    // an AI rate limit further down is the common case) must not duplicate
    // evidence rows on every attempt. Clear this lead's prior contacts and
    // signals before writing fresh ones.
    await db.delete(prospectContacts).where(eq(prospectContacts.leadId, leadId));
    await db.delete(prospectSignals).where(eq(prospectSignals.leadId, leadId));
