...
import { leadRunTasks } from "@/drizzle/schema";
...
export async function createTasks(runId: string, type: TaskType, targets: string[]) {
  ...
  await db.insert(leadRunTasks).values(
    targets.map((target) => ({
...
export async function claimTasks(runId: string, limit: number) {
  const result = await db.execute(sql`
    UPDATE lead_run_tasks
    SET status = 'claimed', claimed_at = now(), attempts = attempts + 1
    WHERE id IN (
      SELECT id FROM lead_run_tasks
...
export async function markTaskDone(taskId: string, resultJson?: unknown) {
  await db
    .update(leadRunTasks)
    .set({ status: "done", finishedAt: new Date(), resultJson: resultJson ?? null })
    .where(eq(leadRunTasks.id, taskId));
}
...
export async function countPendingTasks(runId: string): Promise<number> {
  const [row] = await db
    ...
    .from(leadRunTasks)
    .where(and(eq(leadRunTasks.runId, runId), eq(leadRunTasks.status, "pending")));
  return row?.count ?? 0;
}

export async function countTasksByStatus(runId: string) {
  const rows = await db
    .select({ status: leadRunTasks.status, count: sql<number>`count(*)::int` })
    .from(leadRunTasks)
    .where(eq(leadRunTasks.runId, runId))
    .groupBy(leadRunTasks.status);
  return Object.fromEntries(rows.map((r) => [r.status, r.count]));
}

export async function listTasks(runId: string) {
  return db.select().from(leadRunTasks).where(eq(leadRunTasks.runId, runId)).orderBy(asc(leadRunTasks.createdAt));
}
