import { db } from "@/db";
import { prospectOutbox, prospectSendLog, prospectEnrollments, prospectMailboxes, prospectLeads } from "@/db/schema";
 ...
 * fails every time right now, so every attempt writes to `prospectOutbox` and
...
export async function attemptSend(ctx: SendGateContext & { channel: string; payload: Record<string, unknown> }): Promise<AttemptSendResult> {
  ...
  if (!result.allowed) {
    await db.insert(prospectOutbox).values({
    ...
    if (result.failure.gate !== 1) {
      ...
      await db.insert(prospectSendLog).values({
        id: crypto.randomUUID(),
    ...
    if (result.failure.gate === 2) {
      ...
      await db.update(prospectEnrollments).set({ status: "stopped", stoppedReason: "suppressed" }).where(eq(prospectEnrollments.id, ctx.enrollmentId));
    }
...
export async function recordSentAndScheduleNext(enrollmentId: string, mailboxId: string) {
  await db.update(prospectMailboxes).set({ sentToday: sql`${prospectMailboxes.sentToday} + 1` }).where(eq(prospectMailboxes.id, mailboxId));
  await db
    .update(prospectEnrollments)
    .set({ nextSendAt: new Date(Date.now() + jitterMs()), currentStep: sql`${prospectEnrollments.currentStep} + 1` })
    .where(eq(prospectEnrollments.id, enrollmentId));
}

export async function leadDomain(leadId: string): Promise<string | null> {
  const [lead] = await db.select({ registrableDomain: prospectLeads.registrableDomain }).from(prospectLeads).where(eq(prospectLeads.id, leadId));
