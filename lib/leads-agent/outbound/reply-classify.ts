...
import { replies, enrollments, leads, leadSignals } from "@/drizzle/schema";
...
export async function recordReplyAndStopEnrollments(leadId: string, fromAddress: string, bodyText: string): Promise<string> {
  ...
  await db.insert(replies).values({ id: replyId, leadId, fromAddress, bodyText });
...
export async function classifyAndHandleReply(replyId: string, runId?: string): Promise<void> {
  ...
  const [lead] = await db.select().from(leads).where(eq(leads.id, reply.leadId));
  if (!lead) return;
...
Antwoord met JSON: {"classification":"...","confidence":0.0,"rationale":"korte reden in het Nederlands"}`;
    ...
    const { json } = await callModel<unknown>({ purpose: "reply_classification", model: MODEL_CHEAP, prompt, maxTokens: 300, leadRunId: runId });
  ...
  await db.update(replies).set({ classification, confidence: confidence.toFixed(3), handledAt: new Date() }).where(eq(replies.id, replyId));

  switch (classification) {
    case "optout":
    case "negative":
      if (lead.registrableDomain) await addSuppression("domain", lead.registrableDomain, "optout", rationale);
      await db.update(leads).set({ status: "lost" }).where(eq(leads.id, lead.id));
      break;
    ...
    case "ooo":
      ...
      await db
        ...
        .where(and(eq(enrollments.leadId, lead.id), eq(enrollments.status, "stopped"), eq(enrollments.stoppedReason, "reply_received")));
      break;

    case "positive": {
      await db.update(leads).set({ status: "replied" }).where(eq(leads.id, lead.id));
      const leadSignalRows = await db.select().from(leadSignals).where(eq(leadSignals.leadId, lead.id));
      ...
      if (brief.ok) {
        await db.update(replies).set({ prepBrief: brief.data }).where(eq(replies.id, replyId));
      }
      break;
    }
...
export async function hasAnyReply(leadId: string, registrableDomain: string | null): Promise<boolean> {
  const conditions = [eq(replies.leadId, leadId)];
  if (registrableDomain) conditions.push(ilike(replies.fromAddress, `%@${registrableDomain}`));
  const [hit] = await db.select({ id: replies.id }).from(replies).where(or(...conditions)).orderBy(desc(replies.receivedAt)).limit(1);
  return Boolean(hit);
}
