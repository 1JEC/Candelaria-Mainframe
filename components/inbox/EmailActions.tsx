...
export default function EmailActions({ id, isRead }: { id: string; isRead: boolean }) {
  ...
  return (
    <div className="flex items-center gap-3">
      {!isRead && (
        <button onClick={handleMarkRead} disabled={isPending} className="text-xs text-brand-green hover:text-brand-green-dark disabled:opacity-50">
          Markeer gelezen
        </button>
      )}
      <button onClick={handleArchive} disabled={isPending} className="text-xs text-gray-500 hover:text-gray-800 disabled:opacity-50">
        Archiveren
      </button>
    </div>
  );
