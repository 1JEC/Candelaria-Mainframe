...
import { pageCache } from "@/drizzle/schema";
...
async function queryDoH(domain: string, type: "MX" | "TXT" | "CNAME" | "A", forceRefresh = false): Promise<string[]> {
  ...
  if (!forceRefresh) {
    const [cached] = await db.select().from(pageCache).where(eq(pageCache.urlHash, hash));
  ...
  await db
    ...
    .onConflictDoUpdate({ target: pageCache.urlHash, set: { status: res.status, body: JSON.stringify(records), fetchedAt: new Date() } });
